...
import { Activity, ArrowDownToLine, ArrowRight, ArrowUpRight, BookOpen, Building2, Check, ChevronRight, CircleHelp, Clock3, FileText, Globe2, Languages, Layers3, Loader2, MessageSquareText, Pencil, Plus, Radar, RefreshCw, Rss, ScanSearch, Search, Settings2, ShieldCheck, Sparkles, X } from "lucide-react";
...
import { LANGUAGES, REGIONS, sourceZone, sourcePublisher } from "@/lib/catalog";
import { demoArticles, demoCompany, initialCompanies, initialSources } from "@/lib/demo";
import type { Answer, AppState, Article, Company, Region, Run, Sentiment } from "@/lib/types";

type View = "overview" | "sources" | "ask" | "activity" | "settings";
...
const nav = [{ id: "overview", title: "Overview", icon: ScanSearch }, { id: "ask", title: "Ask Lens", icon: MessageSquareText }, { id: "sources", title: "Sources", icon: Rss }, { id: "activity", title: "Collection activity", icon: Activity }] as const;
...
function Navigation({ state, selected, view, choose, setView, add }: { state: AppState; selected: string; view: View; choose: (id: string) => void; setView: (view: View) => void; add: () => void }) {
  ...
  return <Sidebar className="lens-sidebar" collapsible="offcanvas">
    <SidebarHeader className="lens-sidebar-header"><a href="/" className="brand"><span className="brand-symbol"><Radar size={25} strokeWidth={1.8} /></span><span>company<span className="brand-light">lens</span></span></a><span className="workspace-label">INTELLIGENCE WORKSPACE</span></SidebarHeader>
    <SidebarContent>
...
export default function CompanyLens() {
  ...
  const refresh = useCallback(async (id = selectedRef.current) => {
    ...
    try {
      ...
      if (seq === requestSequence.current) { setState(data); setError(""); }
  ...
  useEffect(() => {
    ...
    api("/api/bootstrap", "POST", {}).then(() => { if (active) return refresh(id); }).catch(e => { if (active) { setError(e.message); setLoading(false); } });
    return () => { active = false; };
  }, [refresh]);
  ...
  const filtered = useMemo(() => state.articles.filter(a => {
    ...
    if (days !== "0" && !company.demo && (!a.publishedAt || Date.parse(a.publishedAt) < Date.now()-Number(days)*86400000)) return false;
    return true;
  }), [state.articles, selected, tone, language, search, days, company.demo]);
          ...
          <div className="overview-grid"><div className="main-column">
            ...
            <Tabs defaultValue="timeline" className="coverage-tabs"><TabsList variant="line"><TabsTrigger value="timeline"><Layers3 size={16} />Story timeline<span className="tab-count">{groups.length}</span></TabsTrigger><TabsTrigger value="comparison"><Globe2 size={16} />Regional comparison</TabsTrigger></TabsList>
              <TabsContent value="timeline"><div className="timeline-panel"><div className="timeline-tools"><div className="coverage-search"><Search size={16} /><Input aria-label="Search coverage" placeholder="Search this coverage…" value={search} onChange={e=>setSearch(e.target.value)} /></div><Picker value={tone} onChange={setTone} label="Filter by tone" options={[{value:"all",label:"All tones"},...Object.entries(toneNames).map(([value,label])=>({value,label}))]} /><Picker value={language} onChange={setLanguage} label="Filter by language" options={[{value:"all",label:"All languages"},...Object.entries(langNames).map(([value,label])=>({value,label}))]} /></div>
                ...
                <div className="timeline-footer">{(search||tone!=="all"||language!=="all"||days!=="0")&&<Button size="sm" variant="ghost" onClick={()=>{setSearch("");setTone("all");setLanguage("all");setDays("0");}}><X size={13}/>Clear filters</Button>}<span>Latest 300 stored articles · {filtered.length} in view</span>{!company.demo && <Picker value={days} onChange={setDays} label="Date range" options={[{value:"0",label:"All dates"},{value:"7",label:"Last 7 days"},{value:"30",label:"Last 30 days"},{value:"90",label:"Last 90 days"}]} />}</div></div></TabsContent>
