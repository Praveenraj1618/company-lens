...
export async function backfillStep(repo: Repository, fetcher: typeof fetch = fetch, loader = loadSource) {
  const job = await nextJob<BackfillData>(repo, 'backfill'); if (!job) return null;
  ...
  try {
    if (d.offset < d.candidates.length) {
      ...
      d.offset++;
    } else if (d.cursor <= d.to) {
      const start = d.cursor, end = new Date(Math.min(Date.parse(start)+6*day,Date.parse(d.to))).toISOString().slice(0,10);
      ...
      d.windows++; d.cursor = new Date(Date.parse(end)+day).toISOString().slice(0,10);
    }
    if (d.cursor > d.to && d.offset >= d.candidates.length) job.status = d.gaps.length ? 'partial' : 'completed';
    job.error = null;
